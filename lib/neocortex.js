...
(function () {
	...
	NeoCortex = {
		...
		/**
		 * @param {Array} input array of arguements from console
		 * @return {None}
		 * @method processInput
		 */
		...
		respondToQuestion: function ( input ) {
			...
			var question = 'I don\'t seem to remember you asking that before, would you like me to Google it, or would you prefer to teach me this? ( Google / Teach )';
		 ...
		 */
		learn : function ( mem ) {
			speech.ask( 'How should I respond to this input?',
				this,
				function ( answer ) {
		...
		getTaskInformation : function( mem ) {
			speech.ask( 'Will this phrase correspond to a task? (Y/n)',
				this,
				function ( answer ) {
		...
		addTaskToMem : function ( mem ) {
			...
			var question = 'Which task is this in relation to? Please choose one. (' + mundane.listTasks().join( '/' ) + ')';
			...
			speech.ask( question,
				...
				function ( answer ) {
					...
					speech.ask( question, this, function ( answer ) {
	
						if ( answer.toLowerCase() === 'y' ) {
	
							// if human has correct task check to see if they would like to add any parameters
							speech.ask( 'Does this particular task have any paramaters? (Y/n)',
								this,
								function ( answer ) {
									if ( answer.toLowerCase() === 'y' ) {
				...
				speech.ask( 'You entered: "' + answer + '", is that correct? (y/N)',
					...
					function ( answer ) {
						...
						if ( answer.toLowerCase() === 'y' ) {
							...
							speech.ask( 'Okay, your parameter was added, would you like to add another? (Y/n)',
								this,
								function ( answer ) {
		...
		/**
		 * moves all short-term memory to long-term memory
		 * @return {None}
		 * @method commitMemories
		 */
		commitMemories : function () {
			...
			try {
				fs.writeFileSync( __dirname + '/memory.json', memories, 'utf8' );
			} catch ( e ) {
				speech.inform( 'Whoops looks like something went wrong! How embarassing!' );
				console.log( e );
			}
